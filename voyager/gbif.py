...
# def gbif_load_years(year_from, year_to, cached=True):
#     if cached:
#         cache_path = CACHE_DIR / f'gbif-{year_from}-{year_to}.csv'

#         try:
#             df = pd.read_csv(
#                 cache_path
#             )
#             logger.info('Loaded GBIF from cache')
#         except FileNotFoundError:
#             logger.info('No GBIF cache found')
#         else:
#             return df

#     gbif = GBIF()
#     gbif = gbif.filter_by_years(year_from, year_to)
#     gbif.to_csv(cache_path)
#     return gbif.df


# def gbif_get_vessels(cached=True):
#     if cached:
#         gbif_cache = GBIFCache('vessel')
#         df = gbif_cache.get()

#     if df.empty:
#         gbif = GBIF()
#         df = gbif.df[gbif.df.vessel.notnull()]
#         gbif_cache.set(df)
...
#     cache_path = CACHE_DIR / f'gbif-{cache_key}.csv'
...
#     @property
#     def path(self):
#         return CACHE_DIR / f'gbif-{self.cache_key}.csv'
