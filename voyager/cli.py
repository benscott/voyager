...
def icoads_search(vessel_name, years, limit):
    ...
    # Have a report, or each vessel ID

    if vessel_name:
        if not years:
            raise click.UsageError('Please select year range -y YYYY-YYYY')
        ...
        try:
            ...
            raise click.UsageError('Please enter year range as YYYY-YYYY')

        years = years_to_range(year_from, year_to)
